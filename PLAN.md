...
- [x] Phase 8 α-edge 整合（可操作分點×標的篩選）✅ **完成（2026-07-09 部署 main、實跑產出）**——修正勝率版三方法論致命傷，落地為每日可操作篩選
  ...
  - **8.9 調參後實跑驗收 ✅（2026-07-09 02:25、c214cac）**：horizons [5,10,20]、benchmark_ready。**H10 值得追蹤 1＝2210 群益期貨**（中性、α+6.0%、OOSα+4.45%、126 事件）＋9 檔近期標的（6488 進場後+28.5%/+24.0%、3081 +10.1%…）；**H20 值得追蹤 2＝2200 元大期貨（中性 α+12.6%）＋5962 日茂南投（佈局 α+7.6%）**＋7 檔近期標的；H5 仍 0（短天期雜訊大）。含中性門檻＋20 日確實讓可操作對象浮現。真實資料經 http server + Playwright 渲染確認 2210 卡與近期標的正確顯示、run_alpha=None 優雅。Phase 8 全案完成、每日自動維護且耐額度波動。
  - **8.10 股票名稱對照（2026-07-09）**：查證 FinMind `dl.taiwan_stock_info(timeout=)`（docstring 逐字確認：無需 stock_id、一次回全市場 `industry_category/stock_id/stock_name/type`，即內部 `_get_stock_id_list` 用的同一 dataset）。collector 新增 `stock_info(stock_id PK, stock_name)` 表＋`ensure_stock_names`（僅空表時抓一次、股名近乎不變不必每日重抓、同代碼多列取最後一筆），置於補價之前呼叫。branch_edge 新增 `load_stock_names`（表不存在時降級回空字典）、`build_live_picks`/`run_horizon` 穿入 `names`，live_picks 附 `stock_name`。index.html 近期標的卡片代碼旁顯示股名。--selftest／整合測試皆加嚴格斷言（注入已知股名驗證正確流通、非空字典空狀態容錯），三次重跑一致。實跑驗收 ✅（2026-07-09 05:25、999833b）：`stock_info` 首抓成功，H10 近期標的正確附名——6488 環球晶、3081 聯亞、5425 台半；H20：6223 旺矽、1710 東聯、2376 技嘉、5351 鈺創。面板 http server + Playwright 渲染確認顯示正確。Phase 8 全案（含股名）完成。
